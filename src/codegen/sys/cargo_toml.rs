...
fn fill_empty(root: &mut Table, env: &Env, crate_name: &str) {
	...
	for ext_lib in &env.config.external_libraries {
		...
		let repo_url = match ext_package.as_str() {
			"cairo-sys-rs" | "gdk-pixbuf-sys" | "gio-sys" | "gobject-sys" | "glib-sys" | "graphene-sys"
			| "pango-sys" | "pangocairo-sys" => "https://github.com/gtk-rs/gtk-rs-core",
			"atk-sys" | "gdk-sys" | "gdkwayland-sys" | "gdkx11-sys" | "gtk-sys" => "https://github.com/gtk-rs/gtk3-rs",
...
fn fill_in(root: &mut Table, env: &Env) {
	{
		let package = upsert_table(root, "package");
		package.entry("build").or_insert_with(|| Value::String("build.rs".into()));
...

	{
		...
		versions.keys().fold(None::<Version>, |prev, &version| {
			let prev_array: Vec<Value> = get_feature_dependencies(version, prev, &env.config.feature_dependencies)
				.iter()
...

	{
		...
		if let Some(lib_name) = ns.package_names.first() {
			...
			meta.entry("name").or_insert_with(|| Value::String(lib_name.clone()));
			meta.entry("version")
...

	{
		...
		docs_rs_metadata.entry("rustc-args").or_insert_with(|| {
			Value::Array(vec![Value::String("--cfg".to_string()), Value::String("docsrs".to_string())])
		});
...
fn upsert_table<S: Into<String>>(parent: &mut Table, name: S) -> &mut Table {
	if let Value::Table(table) = parent.entry(name.into()).or_insert_with(|| Value::Table(toml::map::Map::new())) {
		table
	} else {
		unreachable!()
	}
