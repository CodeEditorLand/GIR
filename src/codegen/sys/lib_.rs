...
fn find_modules(env: &Env) -> Result<Vec<String>> {
	...
	for entry in fs::read_dir(&env.config.auto_path)? {
		...
		let file_stem = file_stem.to_str().expect("Can't convert file name to string").to_owned();
		vec.push(file_stem);
	}
	vec.sort();
...
fn generate_aliases(w: &mut dyn Write, env: &Env, items: &[&Alias]) -> Result<()> {
	...
	for item in items {
		...
		let cfg_condition_ = env.config.objects.get(&full_name).and_then(|obj| obj.cfg_condition.as_ref());
...
fn generate_bitfields(w: &mut dyn Write, env: &Env, items: &[&Bitfield]) -> Result<()> {
	...
	for item in items {
		...
		for member in &item.members {
			let member_config = config.as_ref().map_or_else(Vec::new, |c| c.members.matched(&member.name));
			...
			writeln!(w, "pub const {}: {} = {};", member.c_identifier, item.c_type, val as u32,)?;
		}
		writeln!(w)?;
	}
...
fn generate_constant_cfg_configure(
	...
	let cfg_condition_ = configured_constants.iter().find_map(|f| f.cfg_condition.as_ref());
...
fn generate_constants(w: &mut dyn Write, env: &Env, constants: &[Constant]) -> Result<()> {
	...
	for constant in constants {
		...
		} else if env.library.type_(constant.typ).maybe_ref_as::<Bitfield>().is_some() {
		...
		writeln!(w, "{}pub const {}: {} = {};", comment, constant.c_identifier, type_, value)?;
	}
	if !constants.is_empty() {
		writeln!(w)?;
	}
...
fn generate_enums(w: &mut dyn Write, env: &Env, items: &[&Enumeration]) -> Result<()> {
	...
	for item in items {
		...
		for member in &item.members {
			...
			cfg_condition(w, cfg_condition_, false, 0)?;
			version_condition(w, env, None, version, false, 0)?;
			writeln!(w, "pub const {}: {} = {};", member.c_identifier, item.c_type, member.value,)?;
		}
		writeln!(w)?;
	}
...
fn generate_interfaces_structs(w: &mut dyn Write, env: &Env, interfaces: &[&Interface]) -> Result<()> {
	...
	for interface in interfaces {
		...
		generate_debug_impl(
			w,
			&interface.c_type,
			&format!("write!(f, \"{name} @ {{self:p}}\")", name = interface.c_type),
		)?;
	}
	if !interfaces.is_empty() {
		writeln!(w)?;
...
fn generate_from_fields(w: &mut dyn Write, fields: &fields::Fields, align: Option<u32>) -> Result<()> {
	...
	writeln!(w, "\t\tf.debug_struct(&format!(\"{name} @ {{self:p}}\"))", name = &fields.name)?;
