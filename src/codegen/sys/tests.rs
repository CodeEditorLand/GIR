...
pub fn generate(env: &Env, crate_name: &str) {
	...
	if !manual_h.exists() {
		save_to_file(&manual_h, env.config.make_backup, |w| generate_manual_h(env, &manual_h, w));
	}
...
fn prepare_ctypes(env: &Env) -> Vec<CType> {
	...
	let mut types: Vec<CType> = ns
		...
		.filter_map(|t| match t {
			Type::Record(library::Record { disguised: false, .. }) => prepare_ctype(env, ns, t),
			Type::Alias(_)
			| Type::Class(_)
			| Type::Union(_)
			| Type::Enumeration(_)
...
fn generate_layout_c(env: &Env, path: &Path, w: &mut dyn Write, ctypes: &[CType]) -> io::Result<()> {
	info!("Generating file {:?}", path);
...
#[allow(clippy::write_literal)]
fn generate_constant_c(env: &Env, path: &Path, w: &mut dyn Write, cconsts: &[CConstant]) -> io::Result<()> {
	info!("Generating file {:?}", path);
	general::start_comments(w, &env.config)?;
	writeln!(w)?;
	writeln!(w, "#include \"manual.h\"")?;
