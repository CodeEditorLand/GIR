...
pub fn generate(env: &Env) {
	info!("Generating sys build script for {}", env.config.library_name);
	...
	if !split_build_rs || !path.exists() {
		...
		save_to_file(&path, env.config.make_backup, |w| generate_build_script(w, env, split_build_rs));
	}

	if split_build_rs {
		let path = env.config.target_path.join("build_version.rs");
		info!("Generating file {:?}", path);
		save_to_file(&path, env.config.make_backup, |w| generate_build_version(w, env));
	}
}
...
fn write_version(w: &mut dyn Write, env: &Env, for_let: bool) -> Result<()> {
	...
	for (version, lib_version) in versions.iter().rev() {
		write!(w, "if cfg!({}) {{\n\t\t\"{}\"\n\t}} else ", version.to_cfg(None), lib_version)?;
	}
