...
pub fn generate_records_funcs(w: &mut dyn Write, env: &Env, records: &[&library::Record]) -> Result<()> {
	...
	for record in records {
		...
		generate_object_funcs(w, env, obj, version, &record.c_type, glib_get_type, &record.functions)?;
	}

	Ok(())
}

pub fn generate_classes_funcs(w: &mut dyn Write, env: &Env, classes: &[&library::Class]) -> Result<()> {
	for klass in classes {
		let name = format!("{}.{}", env.config.library_name, klass.name);
		let obj = env.config.objects.get(&name).unwrap_or(default_obj());
		let version = obj.version.or(klass.version);
		generate_object_funcs(w, env, obj, version, &klass.c_type, &klass.glib_get_type, &klass.functions)?;
	}

	Ok(())
}

pub fn generate_bitfields_funcs(w: &mut dyn Write, env: &Env, bitfields: &[&library::Bitfield]) -> Result<()> {
	let intern_str = INTERN.to_string();
	for bitfield in bitfields {
		...
		generate_object_funcs(w, env, obj, version, &bitfield.c_type, glib_get_type, &bitfield.functions)?;
	}

	Ok(())
}

pub fn generate_enums_funcs(w: &mut dyn Write, env: &Env, enums: &[&library::Enumeration]) -> Result<()> {
	let intern_str = INTERN.to_string();
	for en in enums {
		...
		generate_object_funcs(w, env, obj, version, &en.c_type, glib_get_type, &en.functions)?;
	}

	Ok(())
}

pub fn generate_unions_funcs(w: &mut dyn Write, env: &Env, unions: &[&library::Union]) -> Result<()> {
	let intern_str = INTERN.to_string();
	for union in unions {
		...
		generate_object_funcs(w, env, obj, obj.version, c_type, glib_get_type, &union.functions)?;
	}

	Ok(())
}

pub fn generate_interfaces_funcs(w: &mut dyn Write, env: &Env, interfaces: &[&library::Interface]) -> Result<()> {
...
pub fn generate_other_funcs(w: &mut dyn Write, env: &Env, functions: &[library::Function]) -> Result<()> {
	let name = format!("{}.*", env.config.library_name);
	let obj = env.config.objects.get(&name).unwrap_or(default_obj());
	generate_object_funcs(w, env, obj, None, "Other functions", INTERN, functions)
}

fn generate_cfg_configure(w: &mut dyn Write, configured_functions: &[&Function], commented: bool) -> Result<()> {
	let cfg_condition_ = configured_functions.iter().find_map(|f| f.cfg_condition.as_ref());
...
fn generate_object_funcs(
	...
	if write_get_type {
		...
		if configured_functions.iter().all(|f| f.status.need_generate()) {
...
pub fn generate_callbacks(w: &mut dyn Write, env: &Env, callbacks: &[&library::Function]) -> Result<()> {
