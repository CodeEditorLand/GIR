use crate::{
	analysis::{self, conversion_type::ConversionType, rust_type::RustType, try_from_glib::TryFromGlib},
...
pub trait TranslateFromGlib {
	fn translate_from_glib_as_function(&self, env: &Env, array_length: Option<&str>) -> (String, String);
}
		...
		match ConversionType::of(env, self.typ) {
			...
			Scalar => match env.library.type_(self.typ) {
				library::Type::Basic(library::Basic::UniChar) => (
					...
					").expect(\"conversion from an invalid Unicode \
							 value attempted\")"
						.into(),
				),
			...
			Option => {
				let (pre, post) = match &self.try_from_glib {
					...
					TryFromGlib::OptionMandatory => ("try_from_glib(", ").expect(\"mandatory glib value is None\")"),
					other => {
						panic!("Unexpected {other:?} for ConversionType::Option")
					},
				};
			...
			Result { .. } => {
				let (pre, post) = match &self.try_from_glib {
					...
					TryFromGlib::ResultInfallible { .. } => (
						"try_from_glib(",
						").unwrap_or_else(|err| panic!(\"infallible {}\", \
							 err))",
					),
					other => {
						panic!("Unexpected {other:?} for ConversionType::Result")
					},
				};
				(pre.to_string(), post.to_string())
			},
		...
		match self.parameter {
			Some(ref par) => match self.base_tid {
				Some(tid) => {
					...
					let suffix_function =
						if *par.lib_par.nullable { "map(|o| o.unsafe_cast())" } else { "unsafe_cast()" };
				...
				None if self.nullable_return_is_error.is_some() => {
					...
					if let Some(ref msg) = self.nullable_return_is_error {
						...
						(
							...
							format!("{}.ok_or_else(|| {}(\"{}\"))", res.1, use_glib_type(env, "bool_error!"), msg),
						)
					} else {
						res
					}
...
fn from_glib_xxx(transfer: library::Transfer, array_length: Option<&str>) -> (String, String) {
	...
	match (transfer, array_length) {
		...
		(Transfer::None, Some(array_length_name)) => ("from_glib_none_num(".into(), good_print(array_length_name)),
		(Transfer::Full, Some(array_length_name)) => ("from_glib_full_num(".into(), good_print(array_length_name)),
		(Transfer::Container, Some(array_length_name)) => {
			("from_glib_container_num(".into(), good_print(array_length_name))
		},
	}
}
