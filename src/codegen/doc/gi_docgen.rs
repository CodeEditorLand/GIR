...
use crate::{
	...
	codegen::doc::format::{
		gen_alias_doc_link, gen_callback_doc_link, gen_const_doc_link, gen_object_fn_doc_link, gen_property_doc_link,
		gen_signal_doc_link, gen_symbol_doc_link, gen_vfunc_doc_link,
	},
...
fn namespace_type_from_details(link_details: &str, link_type: &str) -> Result<(Option<String>, String), GiDocgenError> {
...
fn namespace_type_method_from_details(
	...
	if len == 1 {
		...
		if res[2].is_empty() {
			...
			Ok((Some(res[0].to_string()), Some(res[1].to_string()), res[2].to_string()))
		}
	} else {
		Err(GiDocgenError::BrokenLinkType(link_type.to_string()))
	}
...
pub(crate) fn replace_c_types(entry: &str, env: &Env, in_type: Option<(&TypeId, Option<LocationInObject>)>) -> String {
	gi_docgen_symbols()
...
fn find_virtual_method_by_name(
	...
	find_method_or_function(
		...
		|o| type_.map_or(true, |t| o.name == t && is_same_namespace(env, namespace, o.type_id)),
		|_| false,
		|_| false,
		|_| false,
		false,
...
fn find_method_or_function_by_name(
	...
	find_method_or_function(
		...
		|o| type_.map_or(true, |t| o.name == t && is_same_namespace(env, namespace, o.type_id)),
		|r| type_.map_or(true, |t| r.name == t && is_same_namespace(env, namespace, r.type_id)),
		|e| type_.map_or(true, |t| e.name == t && is_same_namespace(env, namespace, e.type_id)),
		|f| type_.map_or(true, |t| f.name == t && is_same_namespace(env, namespace, f.type_id)),
		is_class_method,
		false,
	)
}
...
fn is_same_namespace(env: &Env, namespace: Option<&str>, type_id: TypeId) -> bool {
	namespace.as_ref().map_or(MAIN_NAMESPACE == type_id.ns_id, |n| {
		&env.library.namespaces[type_id.ns_id as usize].name == n
	})
}

impl GiDocgen {
	pub fn rust_link(&self, env: &Env, in_type: Option<(&TypeId, Option<LocationInObject>)>) -> String {
		let symbols = env.symbols.borrow();
		match self {
			GiDocgen::Enum { type_, namespace } | GiDocgen::Error { type_, namespace } => {
				env.analysis.enumerations.iter().find(|e| &e.name == type_).map_or_else(
			...
			GiDocgen::Class { type_, namespace } | GiDocgen::Interface { type_, namespace } => env
				...
				.objects
				.values()
				.find(|o| &o.name == type_ && is_same_namespace(env, namespace.as_deref(), o.type_id))
				.map_or_else(
					|| format!("`{}`", ns_type_to_doc(namespace, type_)),
					|info| gen_symbol_doc_link(info.type_id, env),
				),
			GiDocgen::Flag { type_, namespace } => env
				...
				.flags
				.iter()
				.find(|e| &e.name == type_ && is_same_namespace(env, namespace.as_deref(), e.type_id))
				.map_or_else(
					|| format!("`{}`", ns_type_to_doc(namespace, type_)),
					|info| gen_symbol_doc_link(info.type_id, env),
				),
			GiDocgen::Const { type_, namespace } => env
				...
				.map_or_else(|| format!("`{}`", ns_type_to_doc(namespace, type_)), gen_const_doc_link),
			GiDocgen::Property { type_, name, namespace } => env
				.analysis
				.objects
				.values()
				.find(|o| &o.name == type_ && is_same_namespace(env, namespace.as_deref(), o.type_id))
			...
			GiDocgen::Signal { type_, name, namespace } => env
				.analysis
				.objects
				.values()
				.find(|o| &o.name == type_ && is_same_namespace(env, namespace.as_deref(), o.type_id))
			...
			GiDocgen::Struct { namespace, type_ } => env
				...
				.records
				.values()
				.find(|r| &r.name == type_ && is_same_namespace(env, namespace.as_deref(), r.type_id))
				.map_or_else(
					|| format!("`{}`", ns_type_to_doc(namespace, type_)),
					|info| gen_symbol_doc_link(info.type_id, env),
				),
			GiDocgen::Constructor { namespace, type_, name } => env
				.analysis
				...
				.map_or_else(
					...
					|(obj_info, fn_info)| gen_object_fn_doc_link(obj_info, fn_info, env, in_type, type_),
				),
			GiDocgen::Func { namespace, type_, name } => {
				find_method_or_function_by_name(type_.as_deref(), namespace.as_deref(), name, env, in_type, false)
					.unwrap_or_else(|| {
			...
			GiDocgen::Method { namespace, type_, name, is_class_method } => {
				find_method_or_function_by_name(Some(type_), namespace.as_deref(), name, env, in_type, *is_class_method)
					.unwrap_or_else(|| format!("`{}::{}()`", ns_type_to_doc(namespace, type_), name))
			},
			GiDocgen::Callback { namespace, name } => gen_callback_doc_link(&ns_type_to_doc(namespace, name)),
			GiDocgen::VFunc { namespace, type_, name } => {
	...
	fn from_str(item_link: &str) -> Result<Self, Self::Err> {
		...
		if let Some((link_type, link_details)) = item_link.split_once('@') {
			match link_type {
				...
				"ctor" => {
					let (namespace, type_, name) = namespace_type_method_from_details(link_details, "ctor", false)?;
					Ok(Self::Constructor {
						namespace,
						type_: type_.ok_or_else(|| GiDocgenError::BrokenLinkType("ctor".to_string()))?,
						name,
					})
				},
				"enum" => {
				...
				"func" => {
					let (namespace, type_, name) = namespace_type_method_from_details(link_details, "func", true)?;
					Ok(Self::Func { namespace, type_, name })
				},
				"iface" => {
				...
				"method" => {
					let (namespace, type_, name) = namespace_type_method_from_details(link_details, "method", false)?;
					let type_ = type_.ok_or_else(|| GiDocgenError::BrokenLinkType("method".to_string()))?;
					Ok(Self::Method { namespace, is_class_method: type_.ends_with("Class"), type_, name })
				},
				"property" => {
				...
				"vfunc" => {
					let (namespace, type_, name) = namespace_type_method_from_details(link_details, "vfunc", false)?;
					Ok(Self::VFunc {
						namespace,
						type_: type_.ok_or_else(|| GiDocgenError::BrokenLinkType("vfunc".to_string()))?,
						name,
					})
				},
...
mod tests {
	...
	fn test_link_class() {
		assert_eq!(
			...
			Ok(GiDocgen::Class { namespace: None, type_: "Widget".to_string() })
		);
		assert_eq!(
			GiDocgen::from_str("[class@Gdk.Surface]"),
			Ok(GiDocgen::Class { namespace: Some("Gdk".to_string()), type_: "Surface".to_string() })
		);
		assert_eq!(
			GiDocgen::from_str("[class@Gsk.RenderNode]"),
			Ok(GiDocgen::Class { namespace: Some("Gsk".to_string()), type_: "RenderNode".to_string() })
		);
	...
	fn test_link_const() {
		assert_eq!(
			...
			Ok(GiDocgen::Const { namespace: Some("Gdk".to_string()), type_: "KEY_q".to_string() })
		);
	}
	...
	fn test_link_callback() {
		assert_eq!(
			...
			Ok(GiDocgen::Callback { namespace: Some("Gtk".to_string()), name: "MapListModelMapFunc".to_string() })
		);
	}
	...
	fn test_link_enum() {
		assert_eq!(
			...
			Ok(GiDocgen::Enum { namespace: None, type_: "Orientation".to_string() })
		);
	}
	...
	fn test_link_error() {
		assert_eq!(
			...
			Ok(GiDocgen::Error { namespace: Some("Gtk".to_string()), type_: "BuilderParseError".to_string() })
		);
	}
	...
	fn test_link_flags() {
		assert_eq!(
			...
			Ok(GiDocgen::Flag { namespace: Some("Gdk".to_string()), type_: "ModifierType".to_string() })
		);
	}
	...
	fn test_link_iface() {
		assert_eq!(
			...
			Ok(GiDocgen::Interface { namespace: Some("Gtk".to_string()), type_: "Buildable".to_string() })
		);
	}
	...
	fn test_link_struct() {
		assert_eq!(
			...
			Ok(GiDocgen::Struct { namespace: Some("Gtk".to_string()), type_: "TextIter".to_string() })
		);
	}
	...
	fn test_link_vfunc() {
		...
		assert_eq!(
			...
			Ok(GiDocgen::VFunc { namespace: None, type_: "Widget".to_string(), name: "snapshot".to_string() })
		);
	}
	...
	fn test_link_func() {
		assert_eq!(
			...
			Ok(GiDocgen::Func { namespace: Some("Gtk".to_string()), type_: None, name: "init".to_string() })
		);

		assert_eq!(
			GiDocgen::from_str("[func@show_uri]"),
			Ok(GiDocgen::Func { namespace: None, type_: None, name: "show_uri".to_string() })
		);
