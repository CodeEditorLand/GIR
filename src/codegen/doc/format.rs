...
pub fn reformat_doc(input: &str, env: &Env, in_type: Option<(&TypeId, Option<LocationInObject>)>) -> String {
	...
	loop {
		input = match try_split(input, LANGUAGE_BLOCK_BEGIN) {
			(before, Some(after)) => {
				...
				if let (before, Some(after)) = try_split(get_language(after, &mut out), LANGUAGE_BLOCK_END) {
					out.push_str(before);
					out.push_str("\n```");
					after
				} else {
...
fn format(mut input: &str, env: &Env, in_type: Option<(&TypeId, Option<LocationInObject>)>) -> String {
	...
	loop {
		input = match try_split(input, "```") {
			(before, Some(after)) => {
				...
				if !["text", "rust", "xml", "css", "json", "html"].contains(&lang) && after.lines().count() > 1 {
...
fn replace_symbols(input: &str, env: &Env, in_type: Option<(&TypeId, Option<LocationInObject>)>) -> String {
...
fn gdk_gtk() -> &'static Regex {
	...
	static REGEX: OnceLock<Regex> = OnceLock::new();
	REGEX.get_or_init(|| {
		Regex::new(r"`([^\(:])?((G[dts]k|Pango|cairo_|graphene_|Adw|Hdy|GtkSource)\w+\b)(\.)?`").unwrap()
	})
}
...
fn replace_c_types(entry: &str, env: &Env, in_type: Option<(&TypeId, Option<LocationInObject>)>) -> String {
		...
		symbol_name => match &caps[1] {
			...
			"#" => {
				if let Some(member_path) = caps.get(3).map(|m| m.as_str()) {
					...
					type_
				} else if let Some(constant_or_variant) = find_constant_or_variant(symbol_name, env, in_type) {
					warn!(
			...
			"@" => {
				...
				if let Some(type_) = find_type(symbol_name, env) {
					warn!("`{}` matches a type and should use `#` prefix instead of `%`", symbol_name);
					type_
				} else if let Some(constant_or_variant) = find_constant_or_variant(symbol_name, env, in_type) {
					constant_or_variant
				} else if let Some(function) = find_method_or_function_by_ctype(None, symbol_name, env, in_type) {
					function
				} else {
...
fn find_member(
	...
	if !is_signal && !is_property {
		...
		env.analysis.objects.values().find(|o| o.c_type == type_).map(|info| {
...
fn find_constant_or_variant(
	...
	} else if let Some(const_info) = env.analysis.constants.iter().find(|c| c.glib_name == symbol) {
...
const IGNORED_C_TYPES: [&str; 6] = ["gconstpointer", "guint16", "guint", "gunicode", "gchararray", "GList"];
...
fn find_type(type_: &str, env: &Env) -> Option<String> {
	...
	let type_id = if let Some(obj) = env.analysis.objects.values().find(|o| o.c_type == type_) {
		Some(obj.type_id)
	} else if let Some(record) = env.analysis.records.values().find(|r| r.type_(&env.library).c_type == type_) {
		Some(record.type_id)
	} else if let Some(enum_) = env.analysis.enumerations.iter().find(|e| e.type_(&env.library).c_type == type_) {
		Some(enum_.type_id)
	} else if let Some(flag) = env.analysis.flags.iter().find(|f| f.type_(&env.library).c_type == type_) {
		Some(flag.type_id)
	} else {
		None
	};
...
pub(crate) fn find_method_or_function(
	...
	if is_virtual_method {
		if let Some((obj_info, fn_info)) = env.analysis.find_object_by_virtual_method(env, search_obj, search_fn) {
			Some(gen_object_fn_doc_link(obj_info, fn_info, env, in_type, &obj_info.name))
		} else {
			None
		}
	} else if is_class_method {
		if let Some((record_info, fn_info)) = env.analysis.find_record_by_function(env, search_record, search_fn) {
	...
	} else if let Some((obj_info, fn_info)) = env.analysis.find_object_by_function(env, search_obj, search_fn) {
		Some(gen_object_fn_doc_link(obj_info, fn_info, env, in_type, &obj_info.name))
	// or in a record
	} else if let Some((record_info, fn_info)) = env.analysis.find_record_by_function(env, search_record, search_fn) {
		Some(gen_type_fn_doc_link(record_info.type_id, fn_info, env, in_type))
	} else if let Some((enum_info, fn_info)) = env.analysis.find_enum_by_function(env, search_enum, search_fn) {
		Some(gen_type_fn_doc_link(enum_info.type_id, fn_info, env, in_type))
	} else if let Some((flag_info, fn_info)) = env.analysis.find_flag_by_function(env, search_flag, search_fn) {
		Some(gen_type_fn_doc_link(flag_info.type_id, fn_info, env, in_type))
