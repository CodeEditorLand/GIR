	...
	($ty:ident, $enum_var:ident, $useless:expr) => {
		impl ToStripperType for $ty {
			fn to_stripper_type(&self) -> TypeStruct {
				TypeStruct::new(
					SType::$enum_var,
					&format!("connect_{}", nameutil::mangle_keywords(nameutil::signal_to_snake(&self.name))),
				)
			}
		}
	};
...
fn generate_doc(w: &mut dyn Write, env: &Env) -> Result<()> {
	...
	for info in env.analysis.objects.values() {
		if info.type_id.ns_id == MAIN && !env.is_totally_deprecated(None, info.deprecated_version) {
			generators.push((&info.name, Box::new(move |w, e| create_object_doc(w, e, info))));
		}
	}

	for info in env.analysis.records.values() {
		if info.type_id.ns_id == MAIN && !env.is_totally_deprecated(None, info.deprecated_version) {
			generators.push((&info.name, Box::new(move |w, e| create_record_doc(w, e, info))));
		}
	}
	...
	for (tid, type_) in env.library.namespace_types(MAIN) {
		if let LType::Enumeration(enum_) = type_ {
			...
			{
				generators.push((enum_.name.as_str(), Box::new(move |w, e| create_enum_doc(w, e, enum_, tid))));
			}
	...
	if let Some(ref global_functions) = env.analysis.global_functions {
		let functions = ns.functions.iter().filter(|f| f.kind == library::FunctionKind::Global);

		for function in functions {
			if let Some(ref c_identifier) = function.c_identifier {
				let f_info = global_functions.functions.iter().find(move |f| &f.glib_name == c_identifier);
	...
	for constant in &ns.constants {
		...
		let ty = if constant.c_type == "gchar*" { SType::Static } else { SType::Const };
...
fn create_object_doc(w: &mut dyn Write, env: &Env, info: &analysis::object::Info) -> Result<()> {
	...
	let obj = env.config.objects.get(&info.full_name).expect("Object not found");
	...
	write_item_doc(w, &ty, |w| {
		if let Some(doc) = doc_deprecated {
			writeln!(
				w,
				"{}",
				reformat_doc(doc, env, Some((&info.type_id, Some(LocationInObject::Impl))))
			)?;
		}
				...
				match env.library.type_(parent_info.type_id) {
					...
					Type::Interface(iface) => {
						if !iface.properties.is_empty() {
							document_type_properties(env, w, info, &iface.properties, Some(&iface.name))?;
						}
					},
					_ => (),
				}
	...
	if has_trait {
		write_item_doc(w, &ty_ext, |w| {
			...
			let mut implementors = std::iter::once(info.type_id)
				...
				.map(|tid| format!("[`{0}`][struct@crate::{0}]", env.library.type_(tid).get_name()))
				.collect::<Vec<_>>();
			implementors.sort();
	...
	for function in functions {
		...
		let (ty, object_location) =
			if (has_trait || is_manual) && function.parameters.iter().any(|p| p.instance_parameter) && !info.final_type
			{
				if let Some(struct_name) = configured_functions.iter().find_map(|f| f.doc_struct_name.as_ref()) {
					(TypeStruct::new(SType::Impl, struct_name), Some(LocationInObject::Impl))
				}
				// We use "original_name" here to be sure to get the correct object since the "name"
				// field could have been renamed.
				else if let Some(trait_name) = configured_functions.iter().find_map(|f| f.doc_trait_name.as_ref()) {
					(
	...
	for signal in signals {
		...
		let (ty, object_location) = if has_trait {
			if let Some(trait_name) = configured_signals.iter().find_map(|f| f.doc_trait_name.as_ref()) {
	...
	for function in virtual_methods {
		...
		let (ty, object_location) =
			if let Some(trait_name) = configured_virtual_methods.iter().find_map(|f| f.doc_trait_name.as_ref()) {
				(
					TypeStruct::new(SType::Trait, trait_name),
					// Because we cannot sensibly deduce where the docs end up,
					// assume they're outside the docs so that no `Self::` links
		...
		if let Some(c_identifier) = &function.c_identifier {
			let f_info: Option<&analysis::functions::Info> =
				info.virtual_methods.iter().find(|f| &f.glib_name == c_identifier);
			let should_be_documented = f_info.is_some_and(|f| f.should_docs_be_generated(env));
			if !should_be_documented {
				continue;
			}
	...
	for property in properties {
		...
		let (ty, object_location) = if has_trait {
			...
			if let Some(trait_name) = configured_properties.iter().find_map(|f| f.doc_trait_name.as_ref()) {
				(TypeStruct::new(SType::Trait, trait_name), None)
			} else {
				(ty_ext.clone(), Some(LocationInObject::Ext))
			}
...
fn create_bitfield_doc(w: &mut dyn Write, env: &Env, bitfield: &Bitfield, tid: TypeId) -> Result<()> {
	...
	for member in &bitfield.members {
		let generate_doc = config
			.and_then(|m| m.members.matched(&member.name).first().map(|m| m.generate_doc))
			.unwrap_or(true);
	...
	T: FunctionLikeType + ToStripperType,
{
	...
	let self_name: Option<String> = fn_.parameters().iter().find(|p| p.instance_parameter).map(|p| p.name.clone());

	write_item_doc(w, &ty, |w| {
		if let Some(doc) = fn_.doc() {
			writeln!(w, "{}", reformat_doc(&fix_param_names(doc, &self_name), env, in_type))?;
		}
		...
		let mut indices_to_ignore: BTreeSet<_> =
			fn_.parameters().iter().filter_map(|param| param.array_length).collect();
		...
		let no_array_length_params: Vec<_> = fn_
			...
			.iter()
			.enumerate()
			.filter_map(|(indice, param)| (!indices_to_ignore.contains(&(indice as u32))).then_some(param))
		...
		for parameter in in_parameters {
			...
			if let Some(ref doc) = parameter.doc {
				writeln!(w, "## `{}`", nameutil::mangle_keywords(parameter.name.as_str()))?;
				writeln!(w, "{}", reformat_doc(&fix_param_names(doc, &self_name), env, in_type))?;
			}
		}
...
fn create_property_doc(
	...
	if property.doc.is_none() && property.doc_deprecated.is_none() && (property.readable || property.writable) {
		return Ok(());
	}
	let mut v = Vec::with_capacity(2);

	if let Some(getter_name) = getter_name {
		v.push(TypeStruct { parent: parent.clone(), ..TypeStruct::new(SType::Fn, &getter_name) });
	}
	if let Some(setter_name) = setter_name {
		v.push(TypeStruct { parent, ..TypeStruct::new(SType::Fn, &setter_name) });
	}

	for item in &v {
		write_item_doc(w, item, |w| {
			if let Some(ref doc) = property.doc {
				writeln!(w, "{}", reformat_doc(&fix_param_names(doc, &None), env, Some(in_type)))?;
			}
...
fn get_type_trait_for_implements(env: &Env, tid: TypeId) -> String {
	let trait_name = if let Some(&GObject { trait_name: Some(ref trait_name), .. }) =
...
pub fn get_type_manual_traits_for_implements(env: &Env, info: &analysis::object::Info) -> Vec<String> {
	let mut manual_trait_iters = Vec::new();
	for type_id in [info.type_id].iter().chain(info.supertypes.iter().map(|stid| &stid.type_id)) {
	...
	for property in properties {
		...
		if let Some(doc) = &property.doc {
			writeln!(
				...
				"\n\n#### `{}`\n {}\n\n{}",
				property.name,
				reformat_doc(&fix_param_names(doc, &None), env, Some((&info.type_id, None))),
				details.join(" | "),
			)?;
		} else {
