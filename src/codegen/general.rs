...
pub fn start_comments_no_version(w: &mut dyn Write, conf: &Config) -> Result<()> {
	writeln!(w, "// This file was generated by gir (https://github.com/gtk-rs/gir)")?;
	write!(
		w,
		"{}",
		conf.girs_version.iter().fold(String::new(), |mut output, info| {
			writeln!(
				output,
				"// from {}{}",
				info.gir_dir.display(),
				info.get_repository_url().map_or_else(String::new, |url| format!(" ({url})")),
			)
			.unwrap();
			output
		})
...
{}",
		...
		conf.girs_version.iter().fold(String::new(), |mut output, info| {
			match (info.get_repository_url(), info.get_hash()) {
				(Some(url), Some(hash)) => {
					writeln!(output, "{}from {} ({} @ {})", prefix, info.gir_dir.display(), url, hash,)
				},
				(None, Some(hash)) => {
					writeln!(output, "{}from {} (@ {})", prefix, info.gir_dir.display(), hash,)
				},
				_ => {
					writeln!(output, "{}from {}", prefix, info.gir_dir.display())
				},
			}
			.unwrap();
			output
...
pub fn uses(w: &mut dyn Write, env: &Env, imports: &Imports, outer_version: Option<Version>) -> Result<()> {
	writeln!(w)?;

	let mut grouped_imports: BTreeMap<(&str, Option<ImportConditions>), Vec<&str>> = BTreeMap::new();

	for (name, scope) in imports.iter() {
		let (crate_name, import) = name.split_once("::").unwrap();
		let mut scope = scope.clone();

		// The check here is needed to group unneeded version guards and allow
		// grouping those imports
	...
	for ((crate_name, scope), names) in grouped_imports.iter() {
		if !scope.is_none() {
			...
			if !scope.constraints.is_empty() {
				...
				writeln!(w, "#[cfg_attr(docsrs, doc(cfg({})))]", scope.constraints.join(", "))?;
			}
...
pub fn define_fundamental_type(
	...
	external_doc_link(w, env.config.external_docs_url.as_deref(), type_name, &visibility, 1)?;
	writeln!(
		w,
		"\t{visibility} struct \
		 {type_name}(Shared<{sys_crate_name}::{glib_name}>);"
	)?;
	writeln!(w)?;
			...
			.find_map(|p| {
				...
				match type_ {
					Type::Class(class) => Some((
						...
						format!("ptr as *mut {}::{}", parent_sys_crate_name, class.c_type.clone()),
	...
	writeln!(w, "\t\t unsafe {{ from_glib({sys_crate_name}::{glib_func_name}()) }}")?;
	writeln!(w, "\t}}")?;
	writeln!(w, "}}")?;
	Ok(())
}
...
pub fn define_object_type(
	...
	let parents: Vec<StatusedTypeId> = parents.iter().filter(|p| !p.status.ignored()).cloned().collect();
	...
	if parents.is_empty() {
		...
		let prerequisites: Vec<String> = parents.iter().map(|p| format_parent_name(env, p)).collect();
...
fn define_boxed_type_internal(
	...
	writeln!(w)?;
	writeln!(w, "\tmatch fn {{")?;
	let mut_ov = if copy_fn.first_parameter_mut { "mut_override(ptr)" } else { "ptr" };
	writeln!(w, "\t\tcopy => |ptr| {}::{}({}),", sys_crate_name, copy_fn.glib_name, mut_ov)?;
	writeln!(w, "\t\tfree => |ptr| {sys_crate_name}::{free_fn}(ptr),")?;

	if let (Some(init_function_expression), Some(copy_into_function_expression), Some(clear_function_expression)) = (
...
pub fn version_condition_no_doc(
	...
	if should_generate {
		...
		if let Some(s) =
			cfg_condition_string_no_doc(Some(&version.unwrap().to_cfg(namespace_name.as_deref())), commented, indent)
		{
			writeln!(w, "{s}")?;
		}
	}
...
pub fn version_condition_string(
	...
	if should_generate {
		...
		cfg_condition_string(Some(&version.unwrap().to_cfg(namespace_name.as_deref())), commented, indent)
	} else {
		None
	}
}
...
pub fn not_version_condition(
	...
	if let Some(s) =
		version.and_then(|v| cfg_condition_string(Some(&format!("not({})", v.to_cfg(None))), commented, indent))
	{
		writeln!(w, "{s}")?;
	}
	Ok(())
...
pub fn cfg_condition_string_doc(
	...
	cfg_condition.map(|cfg| {
		let comment = if commented { "//" } else { "" };
		format!("{0}{1}#[cfg_attr(docsrs, doc(cfg({2})))]", tabs(indent), comment, cfg,)
	})
}
...
pub fn derives(w: &mut dyn Write, derives: &[Derive], indent: usize) -> Result<()> {
	for derive in derives {
		let s = match &derive.cfg_condition {
			Some(condition) => {
				format!("#[cfg_attr({}, derive({}))]", condition, derive.names.join(", "))
			},
...
pub fn doc_alias(w: &mut dyn Write, name: &str, comment_prefix: &str, indent: usize) -> Result<()> {
	writeln!(w, "{}{}#[doc(alias = \"{}\")]", tabs(indent), comment_prefix, name,)
}
...
pub fn external_doc_link(
	...
	if !visibility.is_public() {
		...
		writeln!(
			w,
			"{}/// This documentation is incomplete due to license \
			 restrictions and limitations on docs.rs. Please have a look at \
			 [our official docs]({}/index.html?search={}) for more \
			 information.",
			tabs(indent),
...
pub fn doc_hidden(w: &mut dyn Write, doc_hidden: bool, comment_prefix: &str, indent: usize) -> Result<()> {
	if doc_hidden {
...
pub fn allow_deprecated(
	...
	if allow_deprecated.is_some() {
		writeln!(w, "{}{}#[allow(deprecated)]", tabs(indent), if commented { "//" } else { "" })
	} else {
		Ok(())
	}
}
