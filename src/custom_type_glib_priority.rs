...
use crate::{analysis::conversion_type::ConversionType, config::WorkMode, library::*, visitors::FunctionsMutVisitor};
...
impl Library {
	pub fn add_glib_priority(&mut self, work_mode: WorkMode) {
		...
		let tid_priority = self.add_type(
			glib_ns_id,
			"Priority",
			Type::Custom(Custom { name: "Priority".to_string(), conversion_type: ConversionType::Scalar }),
		);

		let mut replacer = ReplaceToPriority { tid_priority, tid_int };
		self.namespace_mut(MAIN_NAMESPACE).visit_functions_mut(&mut replacer);
	}
}
