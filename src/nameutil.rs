...
pub(crate) fn set_crate_name_overrides(overrides: HashMap<String, String>) {
	unsafe {
		assert!(CRATE_NAME_OVERRIDES.is_none(), "Crate name overrides already set");
...
pub fn mangle_keywords<'a, S: Into<Cow<'a, str>>>(name: S) -> Cow<'a, str> {
	let name = name.into();
	if let Some(s) = keywords().get(&*name) { s.clone().into() } else { name }
}
...
fn keywords() -> &'static HashMap<&'static str, String> {
	...
	KEYWORDS.get_or_init(|| {
		[
			"abstract", "alignof", "as", "async", "await", "become", "box", "break", "const", "continue", "crate",
			"do", "dyn", "else", "enum", "extern", "false", "final", "fn", "for", "if", "impl", "in", "let", "loop",
			"macro", "match", "mod", "move", "mut", "offsetof", "override", "priv", "proc", "pub", "pure", "ref",
			"return", "Self", "self", "sizeof", "static", "struct", "super", "trait", "true", "try", "type", "typeof",
...
pub fn use_glib_type(env: &crate::env::Env, import: &str) -> String {
	format!("{}::{}", if env.library.is_glib_crate() { "crate" } else { "glib" }, import)
}

pub fn use_glib_if_needed(env: &crate::env::Env, import: &str) -> String {
	format!("{}{}", if env.library.is_glib_crate() { "" } else { "glib::" }, import)
}

pub fn use_gio_type(env: &crate::env::Env, import: &str) -> String {
	format!("{}::{}", if env.library.is_crate("Gio") { "crate" } else { "gio" }, import)
}

pub fn use_gtk_type(env: &crate::env::Env, import: &str) -> String {
	format!("{}::{}", if env.library.is_crate("Gtk") { "crate" } else { "gtk" }, import)
}
