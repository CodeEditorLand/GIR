...
fn git_command(path: impl AsRef<Path>, subcommand: &[&str]) -> Result<Output> {
	let git_path = path.as_ref().to_str().expect("Repository path must be a valid UTF-8 string");
...
pub fn repo_hash(path: impl AsRef<Path>) -> Option<String> {
	...
	if dirty(path) { Some(format!("{hash}+")) } else { Some(hash.into()) }
}
...
fn path_command(path: impl AsRef<Path>, subcommand: &[&str]) -> Option<PathBuf> {
	...
	assert_eq!(output.stdout.pop().map(u32::from).and_then(std::char::from_u32), Some('\n'));
