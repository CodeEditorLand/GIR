...
impl Library {
	pub fn read_file<P: AsRef<Path>>(&mut self, dirs: &[P], libs: &mut Vec<String>) -> Result<(), String> {
		for dir in dirs {
			...
			return parser.document(|p, _| {
				p.element_with_name("repository", |sub_parser, _elem| self.read_repository(dirs, sub_parser, libs))
			});
		}
	...
	fn read_repository<P: AsRef<Path>>(
		...
		parser.elements(|parser, elem| match elem.name() {
			...
			"namespace" => {
				self.read_namespace(parser, elem, std::mem::take(&mut packages), std::mem::take(&mut includes))
			},
	...
	fn read_namespace(
...

		{
			...
			if let Some(s) = elem.attr("shared-library") {
				ns.shared_library = s
					.split(',')
					.filter_map(|x| if !x.is_empty() { Some(String::from(x)) } else { None })
					.collect();
			}
		...
		trace!("Reading {}-{}", ns_name, elem.attr("version").unwrap_or("?"));
	...
	fn read_class(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
		...
		parser.elements(|parser, elem| match elem.name() {
			"constructor" | "function" | "method" => self.read_function_to_vec(parser, ns_id, elem, &mut fns),
			"implements" => self.read_type(parser, ns_id, elem).map(|r| {
				impls.push(r.0);
			}),
			"signal" => self.read_signal(parser, ns_id, elem).map(|s| signals.push(s)),
			"property" => self.read_property(parser, ns_id, elem, &symbol_prefix).map(|p| {
				if let Some(p) = p {
					properties.push(p);
				}
			}),
			...
			"virtual-method" => self.read_virtual_method(parser, ns_id, elem).map(|v| vfns.push(v)),
	...
	fn read_record_start(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
	...
	fn read_record(
		...
		// Records starting with `_` are intended to be private and should not
		// be bound
	...
	fn read_named_union(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
		// Require a name here
	...
	fn read_union(
		...
		parser.elements(|parser, elem| match elem.name() {
			...
			"constructor" | "function" | "method" => self.read_function_to_vec(parser, ns_id, elem, &mut fns),
			"record" => {
				let mut r = match self.read_record(parser, ns_id, elem, parent_name_prefix, parent_ctype_prefix)? {
	...
	fn read_virtual_method(&mut self, parser: &mut XmlParser, ns_id: u16, elem: &Element) -> Result<Function, String> {
		...
		parser.elements(|parser, elem| match elem.name() {
			...
			"return-value" => {
				...
				self.read_parameter(parser, ns_id, elem, true, false).map(|p| ret = Some(p))
			},
			"source-position" => parser.ignore_element(),
			"doc" => parser.text().map(|t| doc = Some(t)),
			"doc-deprecated" => parser.text().map(|t| doc_deprecated = Some(t)),
	...
	fn read_field(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<Field, String> {
		...
		parser.elements(|parser, elem| match elem.name() {
			...
			"callback" => {
				...
				self.read_function(parser, ns_id, elem.name(), elem, true).map(|f| {
		...
		if let Some((tid, c_type, array_length)) = typ {
			Ok(Field { name: field_name.into(), typ: tid, c_type, private, bits, array_length, doc })
		} else {
			Err(parser.fail("Missing <type> element"))
		}
	}

	fn read_named_callback(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
	...
	fn read_interface(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
	...
	fn read_bitfield(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
		...
		parser.elements(|parser, elem| match elem.name() {
			"member" => self.read_member(parser, ns_id, elem).map(|m| members.push(m)),
			"constructor" | "function" | "method" => self.read_function_to_vec(parser, ns_id, elem, &mut fns),
			"doc" => parser.text().map(|t| doc = Some(t)),
			"doc-deprecated" => parser.text().map(|t| doc_deprecated = Some(t)),
			"source-position" => parser.ignore_element(),
	...
	fn read_enumeration(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
		...
		let version = self.read_version(parser, ns_id, elem)?;
		let deprecated_version = self.read_deprecated_version(parser, ns_id, elem)?;
		let error_domain = elem.attr("error-domain").map(|s| ErrorDomain::Quark(String::from(s)));
	...
	fn read_global_function(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
	...
	fn read_constant(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
			...
			"type" | "array" => {
				if inner.is_some() {
					return Err(parser.fail_with_position(
						"Too many <type> inner elements in <constant> \
							 element",
						elem.position(),
					));
				}
				...
				if let Some(c_type) = c_type {
					...
					return Err(parser.fail_with_position("Missing <constant> element's c:type", elem.position()));
				}
				Ok(())
			},
			"doc" => parser.text().map(|t| doc = Some(t)),
		...
		if let Some((typ, c_type, _array_length)) = inner {
			...
			Err(parser.fail_with_position("Missing <type> element inside <constant> element", elem.position()))
		}
	}

	fn read_alias(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<(), String> {
		...
		parser.elements(|parser, elem| match elem.name() {
			...
			"type" | "array" => {
				if inner.is_some() {
					return Err(
						parser.fail_with_position("Too many <type> inner elements in <alias> element", elem.position())
					);
				}
				let (typ, c_type, array_length) = self.read_type(parser, ns_id, elem)?;
				if let Some(c_type) = c_type {
		...
		if let Some((typ, c_type, _array_length)) = inner {
			...
			Err(parser.fail_with_position("Missing <type> element inside <alias> element", elem.position()))
		}
	}

	fn read_member(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<Member, String> {
	...
	fn read_function(
		...
		let finish_func = c_identifier.and_then(|c_identifier| {
			elem.attr("finish-func")
				.map(|finish_func_name| format!("{}{finish_func_name}", c_identifier.strip_suffix(&fn_name).unwrap()))
		});
		...
		parser.elements(|parser, elem| match elem.name() {
			...
			"return-value" => {
				if ret.is_some() {
					return Err(parser.fail_with_position(
						"Too many <return-value> elements inside \
							 <function> element",
						elem.position(),
					));
				}
		...
		// The last argument of a callback is ALWAYS user data, so it has to be
		// marked as such in case it's missing.
		...
		if let Some(ret) = ret {
			...
			Err(parser.fail_with_position("Missing <return-value> element in <function> element", elem.position()))
		}
	}
	...
	fn read_function_if_not_moved(
		...
		self.read_function(parser, ns_id, kind_str, elem, is_callback).and_then(|f| {
	...
	fn read_signal(&mut self, parser: &mut XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<Signal, String> {
		...
		parser.elements(|parser, elem| match elem.name() {
			...
			"return-value" => {
				if ret.is_some() {
					return Err(parser.fail_with_position(
						"Too many <return-value> elements in <signal> \
							 element",
						elem.position(),
					));
				}
		...
		if let Some(ret) = ret {
			...
			Err(parser.fail_with_position("Missing <return-value> element in <signal> element", elem.position()))
		}
	}
	...
	fn read_parameters(
		...
		parser.elements(|parser, elem| match elem.name() {
			"parameter" | "instance-parameter" => self.read_parameter(parser, ns_id, elem, allow_no_ctype, for_method),
			_ => Err(parser.unexpected_element(elem)),
		})
	}
	...
	fn read_parameter(
		...
		if let Some((tid, c_type, mut array_length)) = typ {
			...
			Err(parser
				.fail_with_position(&format!("Missing <type> element in <{}> element", elem.name()), elem.position()))
		}
	}
	...
	fn read_property(
		...
		parser.elements(|parser, elem| {
			match elem.name() {
				"type" | "array" => {
					if typ.is_some() {
						return Err(parser
							.fail_with_position("Too many <type> elements in <property> element", elem.position()));
					}
		...
		if let Some((tid, c_type, _array_length)) = typ {
			...
			Err(parser.fail_with_position("Missing <type> element in <property> element", elem.position()))
		}
	}
	...
	fn read_type(
		...
		let type_name = elem
			.attr("name")
			.or_else(|| if elem.name() == "array" { Some("array") } else { None })
			.ok_or_else(|| parser.fail_with_position("<type> element is missing a name attribute", elem.position()))?;
		...
		if inner.is_empty() || type_name == "GLib.ByteArray" {
			if type_name == "array" {
				Err(parser.fail_with_position("<type> element is missing an inner element type", elem.position()))
			} else if type_name == "gboolean" && c_type.as_deref() == Some("_Bool") {
				Ok((self.find_or_stub_type(ns_id, "bool"), c_type, array_length))
			} else {
				Ok((self.find_or_stub_type(ns_id, type_name), c_type, array_length))
			}
		} else {
	...
	fn read_version(&mut self, parser: &XmlParser<'_>, ns_id: u16, elem: &Element) -> Result<Option<Version>, String> {
	...
	fn read_object_c_type<'a>(&mut self, parser: &XmlParser<'_>, elem: &'a Element) -> Result<&'a str, String> {
		elem.attr("type").or_else(|| elem.attr("type-name")).ok_or_else(|| {
			parser.fail(&format!(
