...
pub struct Trampoline {
	...
	// This field is used for user callbacks in `codegen::function_body_chunk`
	// when generating inner C functions. We need to have the bound name in
	// order to create variables and also to pass to the C function bounds
	// (otherwise it won't compile because it doesn't know how to
	// infer the bounds).
...
pub fn analyze(
	...
	if in_trait || fundamental_type {
		let type_name = RustType::builder(env, type_tid).ref_mode(RefMode::ByRefFake).try_build();
		if fundamental_type {
			bounds.add_parameter("this", &type_name.into_string(), BoundType::AsRef(None), false);
		} else {
	...
	let ret = library::Parameter { nullable: ret_nullable, ..signal.ret.clone() };
...
fn closure_errors(env: &Env, signal: &library::Signal) -> Vec<String> {
	...
	for par in &signal.parameters {
		if let Some(error) = type_error(env, par) {
			errors.push(format!("{} {}: {}", error, par.name, par.typ.full_name(&env.library)));
		}
	}
	if signal.ret.typ != Default::default() {
		if let Some(error) = type_error(env, &signal.ret) {
			errors.push(format!("{} return value {}", error, signal.ret.typ.full_name(&env.library)));
		}
	}
	errors
}
