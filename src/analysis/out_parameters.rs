...
use crate::{
	...
	env::Env,
	library::{self, Basic, Function, INTERNAL_NAMESPACE, Nullable, ParameterDirection, Type, TypeId},
	nameutil,
};
...
pub fn analyze(
	...
	if func.throws {
		let return_strategy = decide_throw_function_return_strategy(env, func_ret, &func.name, configured_functions);
	...
	for lib_par in &func.parameters {
		...
		if can_as_return(env, lib_par) {
			...
			if let Some(c_par) = func_c_params.iter().find(|c_par| c_par.name == lib_par.name) {
	...
	if info.mode == Mode::Combined || info.mode == Mode::Throws(ThrowFunctionReturnStrategy::ReturnResult) {
...
fn decide_throw_function_return_strategy(
	...
	let typ = ret.parameter.as_ref().map(|par| par.lib_par.typ).unwrap_or_default();
...
pub fn use_function_return_for_result(
	...
	let use_return_for_result = configured_functions.iter().find_map(|f| f.ret.use_return_for_result.as_ref());
