...
fn get_node<'a>(library: &Library, hier: &'a mut HashMap<TypeId, Node>, tid: TypeId) -> Option<&'a mut Node> {
	if hier.contains_key(&tid) {
		return hier.get_mut(&tid);
	}

	let direct_supers: Vec<TypeId> = match library.type_(tid) {
		Type::Class(Class { parent, implements, .. }) => parent.iter().chain(implements.iter()).copied().collect(),
	...
	hier.insert(tid, Node { supers, subs: HashSet::new() });
	hier.get_mut(&tid)
}
