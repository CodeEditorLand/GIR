...
pub fn analyze(
	...
	for &super_tid in env.class_hierarchy.supertypes(type_id) {
		...
		parents.push(StatusedTypeId { type_id: super_tid, name: env.library.type_(super_tid).get_name(), status });
		...
		if !status.ignored() && super_tid.ns_id == namespaces::MAIN && !add_parent_types_import {
			if let Ok(rust_type) = RustType::try_new(env, super_tid) {
				...
				if let Some(parent_version) = env.analysis.objects.get(&full_name).and_then(|info| info.version) {
