...
impl Parameters {
	...
	pub fn analyze_return(&mut self, env: &Env, ret: &Option<analysis::Parameter>) {
		let ret_data = ret.as_ref().map(|r| (r.lib_par.array_length, &r.try_from_glib));
...
pub fn analyze(
	...
	for (pos, par) in function_parameters.iter().enumerate() {
		...
		if async_func && pos >= correction_instance && to_remove.contains(&(pos - correction_instance)) {
		...
		let move_ = configured_parameters.iter().find_map(|p| p.move_).unwrap_or_else(|| {
		...
		let ref_mode = RefMode::without_unneeded_mut(env, par, immutable, in_trait && par.instance_parameter);
		...
		let transformation_type = match conversion {
			ConversionType::Direct => {
				if par.c_type != "GLib.Pid" {
					...
					TransformationType::ToGlibScalar { name, nullable, needs_into: false }
				}
			},
			ConversionType::Scalar => TransformationType::ToGlibScalar { name, nullable, needs_into: false },
			ConversionType::Option => {
				let needs_into = match try_from_glib {
					...
					other => {
						unreachable!("{:?} inconsistent / conversion type", other)
					},
				};
				TransformationType::ToGlibScalar { name, nullable: Nullable(false), needs_into }
			},
			...
			ConversionType::Pointer => TransformationType::ToGlibPointer {
				...
				pointer_cast: if matches!(env.library.type_(typ), library::Type::CArray(_)) && par.c_type == "gpointer"
				{
		...
		let mut transformation = Transformation { ind_c, ind_rust, transformation_type };
...
fn get_length_type(env: &Env, array_name: &str, length_name: &str, length_typ: TypeId) -> TransformationType {
