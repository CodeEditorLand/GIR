...
impl Default for Bounds {
	fn default() -> Bounds {
		Bounds {
			unused: (TYPE_PARAMETERS_START..).take_while(|x| *x <= 'Z').collect(),
			used: Vec::new(),
			lifetimes: Vec::new(),
		}
	}
...
impl Bounds {
	pub fn add_for_parameter(
		...
		let type_name = RustType::builder(env, par.typ).ref_mode(RefMode::ByRefFake).try_build();
		if type_name.is_err() {
			return (None, None);
		}
		...
		if !par.instance_parameter && par.direction != ParameterDirection::Out {
			if let Some(bound_type) = Bounds::type_for(env, par.typ) {
				...
				if r#async && (par.name == "callback" || par.name.ends_with("_callback")) {
					...
					if let Some(function) = find_function(env, &finish_func_name) {
						...
						// from here either.
						let mut out_parameters = find_out_parameters(env, function, configured_functions);
					...
					if let Type::Function(_) = env.library.type_(par.typ) {
						...
						type_string = rust_ty.try_from_glib(&par.try_from_glib).try_build().into_string();
						let bound_name = *self.unused.front().unwrap();
						callback_info = Some(CallbackInfo {
							callback_type: type_string.clone(),
			...
			if let Some(bound_type) = Bounds::type_for(env, par.typ) {
				ret = Some(Bounds::get_to_glib_extra(&bound_type, *par.nullable, true, par.move_));
			}
		}
	...
	pub fn type_for(env: &Env, type_id: TypeId) -> Option<BoundType> {
		...
		match env.library.type_(type_id) {
			...
			Type::Class(Class { is_fundamental: true, .. }) => Some(AsRef(None)),
	...
	pub fn add_parameter(&mut self, name: &str, type_str: &str, mut bound_type: BoundType, r#async: bool) {
...
fn find_out_parameters(
	...
	function
		...
		.enumerate()
		.filter(|&(index, param)| {
			Some(index) != index_to_ignore && param.direction == ParameterDirection::Out && param.name != "error"
		})
		.map(|(_, param)| {
