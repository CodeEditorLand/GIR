...
pub fn analyze(
	...
	for prop in props {
		...
		if supertypes_props.iter().any(|p| p.name == prop.name && p.typ == prop.typ) {
			continue;
		}
...
fn analyze_property(
	...
	if readable {
		for check_get_func_name in check_get_func_names {
			let (has, version) =
				Signature::has_for_property(env, &check_get_func_name, true, prop.typ, signatures, deps);
			if has {
	...
	if writable {
		let (has, version) = Signature::has_for_property(env, &set_func_name, false, prop.typ, signatures, deps);
		if has {
	...
	let getter_func = functions
		.iter()
		.find(|f| f.get_property.as_ref() == Some(&prop.name) && Some(&f.name) == prop.getter.as_ref());
	let setter_func = functions
		.iter()
		.find(|f| f.set_property.as_ref() == Some(&prop.name) && Some(&f.name) == prop.setter.as_ref());

	let has_getter = getter_func.is_some_and(|g| matches!(g.status, GStatus::Generate | GStatus::Manual));
	let has_setter = setter_func.is_some_and(|s| matches!(s.status, GStatus::Generate | GStatus::Manual));
...
pub fn get_property_ref_modes(env: &Env, prop: &library::Property) -> (RefMode, RefMode, library::Nullable) {
