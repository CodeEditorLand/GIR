...
impl TomlHelper for toml::Value {
	...
	fn as_result_str<'a>(&'a self, option: &'a str) -> Result<&'a str, String> {
		self.as_str()
			.ok_or_else(|| format!("Invalid `{}` value, expected a string, found {}", option, self.type_str()))
	}

	fn as_result_vec<'a>(&'a self, option: &'a str) -> Result<&'a Vec<Self>, String> {
		self.as_array()
			.ok_or_else(|| format!("Invalid `{}` value, expected a array, found {}", option, self.type_str()))
	}

	fn as_result_bool<'a>(&'a self, option: &'a str) -> Result<bool, String> {
		self.as_bool()
			.ok_or_else(|| format!("Invalid `{}` value, expected a boolean, found {}", option, self.type_str()))
	}
}
