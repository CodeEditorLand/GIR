...
impl Parse for VirtualMethod {
	fn parse(toml: &Value, object_name: &str) -> Option<Self> {
		...
		let doc_ignore_parameters = toml
			...
			.map(|v| v.iter().filter_map(|v| v.as_str().map(String::from)).collect())
			.unwrap_or_default();
		let doc_trait_name = toml.lookup("doc_trait_name").and_then(Value::as_str).map(ToOwned::to_owned);
		let unsafe_ = toml.lookup("unsafe").and_then(Value::as_bool).unwrap_or(false);
		let rename = toml.lookup("rename").and_then(Value::as_str).map(ToOwned::to_owned);
		if !check_rename(&rename, object_name, &ident) {
			return None;
		}
		let bypass_auto_rename = toml.lookup("bypass_auto_rename").and_then(Value::as_bool).unwrap_or(false);
