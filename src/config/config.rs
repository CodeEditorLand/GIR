	...
	for component in path.as_ref().components() {
		match (component, parts.last()) {
			...
			(Component::ParentDir, Some(_)) => {
				parts.pop().expect("Cannot navigate outside of base directory!");
			},
			(c, _) => parts.push(c),
		}
	}
...
impl Config {
		...
		W: Into<Option<WorkMode>>,
	{
		...
		let toml = match read_toml(&config_file) {
			Ok(toml) => toml,
			Err(e) => {
				return Err(format!("Error while reading \"{}\": {}", config_file.display(), e));
			},
		};
		...
		let work_mode = match work_mode.into() {
			...
			None => {
				let s = match toml.lookup_str("options.work_mode", "No options.work_mode") {
					Ok(s) => s,
					Err(e) => {
						return Err(format!("Invalid toml file \"{}\": {}", config_file.display(), e));
					},
				};
				WorkMode::from_str(s)?
			},
		...
		let mut girs_dirs: Vec<PathBuf> =
			girs_dirs.iter().filter(|x| !x.is_empty()).map(|x| PathBuf::from(&x)).collect();
		if girs_dirs.is_empty() {
			let dirs = toml.lookup_vec("options.girs_directories", "No options.girs_directories")?;
			for dir in dirs {
				let dir = dir
					.as_str()
					.ok_or_else(|| "options.girs_dirs expected to be array of string".to_string())?;
		...
		if work_mode == WorkMode::Normal && auto_path.exists() {
			std::fs::remove_dir_all(&auto_path).map_err(|e| format!("remove_dir_all failed: {e:?}"))?;
		}
		...
		let trust_return_value_nullability = match toml.lookup("options.trust_return_value_nullability") {
		...
		let mut objects = toml
			.lookup("object")
			.map(|t| gobjects::parse_toml(t, concurrency, generate_builder, trust_return_value_nullability))
			.unwrap_or_default();
	...
	pub fn filter_version(&self, version: Option<Version>) -> Option<Version> {
		version.and_then(|v| if v > self.min_cfg_version { Some(v) } else { None })
	}
...
fn read_toml<P: AsRef<Path>>(filename: P) -> Result<toml::Value, String> {
	...
	let input = fs::read(&filename).map_err(|e| format!("Failed to read file \"{:?}\": {}", filename.as_ref(), e))?;

	let input =
		String::from_utf8(input).map_err(|e| format!("File is not valid UTF-8 \"{:?}\": {}", filename.as_ref(), e))?;

	toml::from_str(&input).map_err(|e| format!("Invalid toml format in \"{}\": {}", filename.as_ref().display(), e))
}
...
fn read_extra_versions(toml: &toml::Value) -> Result<Vec<Version>, String> {
	match toml.lookup("options.extra_versions") {
		Some(a) => a
			...
			.map(|v| {
				v.as_str()
					.ok_or_else(|| "options.extra_versions expected to be array of string".to_string())
			})
...
fn read_lib_version_overrides(toml: &toml::Value) -> Result<HashMap<Version, Version>, String> {
	...
	for o in v {
		let cfg = o.lookup_str("version", "No version in lib_version_overrides")?.parse()?;
		let lib = o
...
fn read_feature_dependencies(toml: &toml::Value) -> Result<HashMap<Version, Vec<String>>, String> {
	...
	let mut map = HashMap::with_capacity(v.len());
	for o in v {
		let cfg = o.lookup_str("version", "No version in feature_dependencies")?.parse()?;
